...
aux_lrs = [1e-6, 5e-6, 1e-5, 5e-5, 1e-4, 5e-4, 1e-3]
dampings = [0.1, 0.15, 0.2, 0.25, 0.3]
precondition_auxs = [True, False]

for aux_lr in aux_lrs:
    for damping in dampings:
        for precondition_aux in precondition_auxs:
            ...
            log_dir = f"runs/experiment_scan/CIFAR_ResNet18_pretrained_fishleg/lr={lr}_batch_size={batch_size}_lambda={weight_decay}_aux_lr={aux_lr}_damping={damping}_update_aux={update_aux_every}_method={method}_aux_eps={aux_eps}_precond={precondition_aux}/{datetime.now().strftime('%Y%m%d-%H%M%S')}"
            ...
            print("Finished Training with lr=", lr, "damping=", damping, "precondition_aux=", precondition_aux)
